...
*E.g.* if the reaction was $\require{mhchem}\ce{ 2A -> B }$ and the rate equation was $-r_A = k1C_A^2$
...
Now if we plot $1/(-r_A)$ vs $x_A$, the ratio of the reactor volume to the flow rate for a CSTR is the rectangle where the desired fractional conversion intersects the reaction rate. for a first-order, isothermal reaction, this looks like the following.
```{r, echo=params$code}
k_1 <- 0.05
C_A0 <- 5 #M
t <- seq(0, 100, 0.1)
